...
struct YaamavaGameInitialView: View {
    private var gameResourceURL: URL { URL(string: "https://yaamagame.top/get")! }

    var body: some View {
        ZStack {
            Color(hex: "#000")
                .ignoresSafeArea()
            YaamavaEntryScreen(loader: .init(resourceURL: gameResourceURL))
        }
    }
}

#Preview {
    YaamavaGameInitialView()
}
